...
public class Inventory : MonoBehaviour
{
    ...
    void hold()
    {
        ...
        {
            if (stuffName[holdNumber - 1] == "apple") spawnItem = Instantiate(apple, item.transform);
            if (stuffName[holdNumber - 1] == "cookie") spawnItem =Instantiate(cookie, item.transform);
            if (stuffName[holdNumber - 1] == "water") spawnItem = Instantiate(water, item.transform);
            if (stuffName[holdNumber - 1] == "pie") spawnItem = Instantiate(pie, item.transform);

            if(spawnItem.GetComponent<Collider>() != null) spawnItem.GetComponent<Collider>().enabled = false;
